...
<div class="cart-item flex gap-5 mb-4 pb-8 border-b border-gray-100 last:border-0 last:mb-0 last:pb-0">
    ...
    <div class="item-info flex-1 flex flex-col">
        <div class="flex justify-between items-start">
            ...
            <button class="text-gray-400 hover:text-black transition-colors" onclick="removeFromCart('{{ $item['key'] }}')">&times;</button>
        </div>
        ...
        <div class="item-qty-price flex justify-between items-end mt-4">
            <div class="qty-controls flex gap-3 text-xs border border-gray-200 px-3 py-1 items-center">
                <span class="cursor-pointer px-1 hover:text-gray-500" onclick="updateCartQty('{{ $item['key'] }}', {{ $item['quantity'] - 1 }})">&minus;</span>
                <span class="w-4 text-center">{{ $item['quantity'] }}</span>
                <span class="cursor-pointer px-1 hover:text-gray-500" onclick="updateCartQty('{{ $item['key'] }}', {{ $item['quantity'] + 1 }})">&plus;</span>
            </div>
