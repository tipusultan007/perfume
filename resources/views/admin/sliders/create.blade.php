        ...
        <form action="{{ route('admin.sliders.store') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" placeholder="e.g. Luxury Scent Collection"
                ...
                <div>
                    ...
                    <input type="text" name="subtitle" value="{{ old('subtitle') }}" placeholder="e.g. New Arrivals 2026"
            ...
            <div class="pt-8 border-t border-slate-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div>
                        ...
                        <div class="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-xl">
                            <input type="color" name="bg_color" value="{{ old('bg_color', '#FDFCF8') }}" class="w-12 h-12 rounded-lg cursor-pointer border-2 border-white shadow-sm">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="ui_theme" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:border-slate-900 focus:ring-4 focus:ring-slate-900/5 outline-none transition-all text-[11px] text-slate-900 font-bold uppercase tracking-widest appearance-none cursor-pointer">
                                <option value="dark" {{ old('ui_theme') == 'dark' ? 'selected' : '' }}>Dark (For Light BGs)</option>
                                <option value="light" {{ old('ui_theme') == 'light' ? 'selected' : '' }}>Light (For Dark BGs)</option>
                            </select>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-xl">
                            <input type="color" name="accent_color" value="{{ old('accent_color', '#D4AF37') }}" class="w-12 h-12 rounded-lg cursor-pointer border-2 border-white shadow-sm">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
                    ...
                    <div class="p-4 bg-slate-50 border border-slate-100 rounded-xl">
                        ...
                        <input type="color" name="{{ $field }}" value="{{ old($field, '#111827') }}" class="w-full h-8 rounded border-none cursor-pointer shadow-sm">
                    </div>
                    @endforeach
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10 mt-10 p-8 bg-slate-900 rounded-2xl shadow-xl shadow-slate-900/10">
                    <div>
                        ...
                        <input type="text" name="price" value="{{ old('price') }}" placeholder="e.g. $145.00"
                    ...
                    <div>
                        ...
                        <input type="text" name="top_notes" value="{{ old('top_notes') }}" placeholder="e.g. Mint, Bergamot & Ozone"
