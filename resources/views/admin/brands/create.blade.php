...
<div class="max-w-2xl">
    <div class="flex items-center gap-4 mb-10">
        ...
        <h3 class="font-serif text-xl">New Brand</h3>
    </div>
        ...
        <form action="{{ route('admin.brands.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div>
                ...
                <textarea name="description" rows="4" 
                    class="w-full py-4 border border-black/10 focus:border-luxury-black outline-none transition-colors text-sm bg-transparent px-4">{{ old('description') }}</textarea>
                @error('description')
                    <span class="text-red-500 text-[10px] mt-2 block tracking-widest">{{ $message }}</span>
                @enderror
            </div>
