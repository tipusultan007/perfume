...
<div class="bg-white p-10 shadow-sm border border-black/[0.03] max-w-2xl">
    <h3 class="font-sans font-semibold text-lg mb-8">Edit Announcement</h3>
    ...
    <form action="{{ route('admin.announcements.update', $announcement) }}" method="POST">
        ...
        <div class="space-y-6">
            <div>
                ...
                <textarea name="content" rows="3" class="w-full bg-gray-50 border border-black/5 p-4 text-sm focus:outline-none focus:border-luxury-accent transition-colors" required placeholder="Enter announcement text...">{{ old('content', $announcement->content) }}</textarea>
                @error('content')
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="display_order" value="{{ old('display_order', $announcement->display_order) }}" class="w-full bg-gray-50 border border-black/5 p-4 text-sm focus:outline-none focus:border-luxury-accent transition-colors">
                    @error('display_order')
                        <p class="text-red-500 text-[10px] mt-1 uppercase tracking-tighter">{{ $message }}</p>
                    @enderror
                </div>
