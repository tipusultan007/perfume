...
<div class="max-w-2xl">
    <div class="flex items-center gap-4 mb-10">
        ...
        <h3 class="font-bold text-2xl text-slate-900">New Attribute</h3>
    </div>
    ...
    <div class="bg-white border border-slate-200 p-10 rounded-xl shadow-sm">
        <form action="{{ route('admin.attributes.store') }}" method="POST" class="space-y-8">
            @csrf
            <div>
                <label class="block text-[10px] uppercase tracking-widest mb-3 text-slate-500 font-bold">Attribute Name (e.g., Color, Size)</label>
                ...
                <div id="values-container" class="space-y-3">
                    <div class="flex items-center gap-3">
                        <div class="relative flex-1">
                            <input type="text" name="values[]" required placeholder="e.g. 100ml"
                                class="w-full px-5 py-4 bg-white border border-slate-200 rounded-lg focus:border-slate-900 focus:ring-4 focus:ring-slate-900/5 outline-none transition-all text-sm text-slate-900 font-medium">
                        </div>
                        <button type="button" class="w-12 h-12 flex items-center justify-center text-slate-300 cursor-not-allowed opacity-20">
